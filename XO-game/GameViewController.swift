...
class GameViewController: UIViewController {
    ...
    private let xoInvoker = XOInvoker()
    ...
    @IBAction func restartButtonTapped(_ sender: UIButton) {
        ...
        if playFiveMarks {
            xoInvoker.clear()
            self.firstPlayerTurnLabel.text = "1st player - X, moves left " + String(5 - self.xoInvoker.xCommands.count)
            self.secondPlayerTurnLabel.text = "2nd player - O, moves left " + String(5 - self.xoInvoker.oCommands.count)
        }
    }
    ...
    override func viewDidLoad() {
        ...
        gameboardView.onSelectPosition = { [weak self] position in
            ...
            // игра по 5 ходов
            if self.playFiveMarks {
                
                if (self.xoInvoker.xCommands.count + self.xoInvoker.oCommands.count) <= 10 {
                    guard let player = self.currentState.currentPlayer else { return }
                    if !self.gameboard.contains(player: player, at: position)   {
                        self.xoInvoker.addComand(сommand: XOCommand(player: player, position: position))
                        
                        if self.xoInvoker.xCommands.count == 5 && self.xoInvoker.oCommands.count == 0 {
                            if let nextPlayer = self.currentState.currentPlayer?.next {
                                let markView: MarkView = nextPlayer == .first ? self.firstPlayerMarkViewPrototype.copy() : self.secondPlayerMarkViewPrototype.copy()
                                self.currentState = PlayerInputState(player: nextPlayer, gameViewController: self, markViewPrototype: markView)
                            }
                        }
                        
                        if self.xoInvoker.oCommands.count == 5  {
                            
                            self.xoInvoker.runCommands(gameViewController: self)
                            
                            self.xoInvoker.endOfGame = {
                                DispatchQueue.main.async {
                                    if let winner = self.referee.determineWinner() {
                                        
                                        if self.referee.doesPlayerHaveWinningCombination(.first) && self.referee.doesPlayerHaveWinningCombination(.second) {
                                            
                                            self.currentState = GameFinishedState(player: nil, gameViewController: self)

                                        } else {
                                        
                                       
                                        self.currentState = GameFinishedState(player: winner, gameViewController: self)
                                        }
                        ...
                        print("Клетка уже занята текущим игроком")
                    }
                }
                // игра по одному ходу
            } else {
                ...
                if !busyCell {
                    ...
                    if !self.playWithComputer {
                        ...
                        //  print(self.gameboard.positions)
